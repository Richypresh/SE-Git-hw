...
I made the repo, cloned it to my computer, and pushed my first commit with `hello_world.py`. Then I made a new branch called `feature-1`, added `apple.py` on it, and pushed that branch separately. After that, I opened a pull request to merge `feature-1` into `main` and merged it once it was reviewed.


## The Merge Conflict (this part took me a while)

I had to intentionally create a merge conflict to see how to fix one. I changed the same line in `hello_world.py` on two different branches, and when I tried to merge them Git stopped me with:
...
| #2 | me | Review the Hello World program and verify that the program runs correctly and follows the assignment requirements. | Ran `python hello_world.py`, confirmed it printed "Hello, World!" as required, and confirmed the code matched the assignment requirements | Closed |
